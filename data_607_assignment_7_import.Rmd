...
cuny_mart_parquet <- open_dataset(pqpath) |> collect()
...
The following code exports the original dataframe to JSON format. 
...
The following code exports the data from the prepared dataframe to an html file.
...
The following code imports an xml file and converts it into a dataframe for use in R. XML is an Extensible Markup language and is useful for carrying and storing data. It allows customization through tags and is hierarchical. However, it tends to have larger file sizes and slower queries so it is not useful for big data.
